...
internal class LoggerPrinter : Printer {
    ...
    override fun log(
        ...
        if (throwable != null && updatedMessage != null) {
            updatedMessage += " : " + Utils.getStackTraceString(throwable)
        }
        if (throwable != null && updatedMessage == null) {
            updatedMessage = Utils.getStackTraceString(throwable)
        }
        if (Utils.isEmpty(updatedMessage)) {
            updatedMessage = "Empty/NULL log message"
        }
        for (adapter in logAdapters) {
            if (adapter.isLoggable(priority, tag)) {
                adapter.log(priority, tag, updatedMessage!!)
            }
        }
    }
